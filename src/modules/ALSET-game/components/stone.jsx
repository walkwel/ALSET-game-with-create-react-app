...
export default class Stone extends Component {
  ...
  getWrapperStyles() {
    return {
      ...
      transform:
        'translate(' +
        this.props.store.stonesData[this.props.index].x * this.context.scale +
        'px, ' +
        this.props.store.stonesData[this.props.index].y * this.context.scale +
        'px) translateZ(0)',
  ...
  render() {
    return (
      ...
      >
        <TileMap style={{ top: 0, left: 0 }} src={StoneImg} rows={1} columns={1} tileSize={64} layers={[[1]]} />
      </div>
    );
  }
}
